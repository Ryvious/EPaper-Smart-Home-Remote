...
static lv_obj_t* menu_fragment_create_obj(lv_fragment_t* self, lv_obj_t* parent) {
    ...
    for (view_t view : *fragment->views) {
        lv_obj_t* view_label = lv_label_create(content);
        lv_label_set_text(view_label, view.name.c_str());

        lv_obj_set_style_text_font(view_label, &lv_font_montserrat_44, LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_obj_set_width(view_label, lv_pct(100));
        lv_obj_set_style_text_align(view_label, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_obj_align(view_label, LV_ALIGN_CENTER, 0, 0);
        lv_obj_add_flag(view_label, LV_OBJ_FLAG_CLICKABLE);

        lv_obj_add_event_cb(view_label, [](lv_event_t* e) {
            static_cast<menu_fragment_t*>(lv_event_get_user_data(e))->callback(lv_label_get_text(e->target));
